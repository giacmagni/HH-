...
void DrawHisto	(TTree * signal, TTree * background, string * var, int nbin, float min, float max, int N ){	
	...
	for(int i = 0; i < N; i++){
		...
		if ( i == 0 )	dist->SetTitle(" Invariant mass of b bbar; Mass [GeV]; Events");
		if ( i == 1 )	dist->SetTitle(" Invariant mass of jet components ; Mass [GeV]; Events");
		if ( i == 2 )	dist->SetTitle(" Visible Invariant mass of WW ; Mass [GeV]; Events");
	}
	return;
}
